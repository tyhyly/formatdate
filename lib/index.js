...
/**
 * @params: date 目前暂时只支持'2016-01-01'格式
 * @return: 季度(格式化为'2016年Q1')
 **/
export function formatDateToQuarter(date) {
    let d = '';
    if(typeof date === 'string') {
        if(Number(date.slice(5,7)) == 1) {
            d = date.slice(0,4) + '年Q1';
        }else if(Number(date.slice(5,7)) == 4) {
            d = date.slice(0,4) + '年Q2';
        }else if(Number(date.slice(5,7)) == 7) {
            d = date.slice(0,4) + '年Q3';
        }else if(Number(date.slice(5,7)) == 10) {
            d = date.slice(0,4) + '年Q4';
        }
    }
    return d;
}
...
/**
 * @params: date 目前暂时只支持'2016-01-01'格式
 * @return: 季度(格式化为'2016年01月')
 **/
export function formatDateToMonth(date) {
    let d = '';
    if(typeof date === 'string') {
        d = date.slice(0,4) + '年' + date.slice(5, 7) + '月';
    }
    return d;
}